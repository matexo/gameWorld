(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('gameWorldApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-gameWorldApp-error');
                var entityKey = httpResponse.headers('X-gameWorldApp-params');
                if (errorHeader) {
